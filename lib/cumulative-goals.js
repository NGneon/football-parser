/**
 * Голы в скобках — накопительно по сезону ПЕРЕД текущим матчем
 * (без учёта голов в этом матче).
 */

const { normalizeName, namesMatch } = require('./match-goals')
const { isBadPlayer } = require('./lineup-quality')
...
function applyCumulativeToLineup(lineup, team, tally) {
	const raw = lineup || []
	const list = raw
		.map(item => {
			const base =
				typeof item === 'string'
					...
					: { ...item }
			if (!base.name || isBadPlayer(base)) return null
			return {
				...base,
...
function attachCumulativeGoalsToMatchList(sortedMatches) {
	...
	for (const match of sortedMatches) {
		...
		// Сначала показываем накопленные голы ДО матча, затем учитываем голы матча
