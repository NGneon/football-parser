...
const { buildSummaryUrl, fetchLineups, isCompleteLineup } = require('./lineups')
...
async function parseMatchFull(page, matchUrl, match) {
	...
	try {
		...
		const lineups = await fetchLineups(page, matchUrl, sleep)

		if (!isCompleteLineup(lineups)) {
			console.warn(
				`  ⚠ неполный состав: ${match.team1 || ''} ${lineups.homePlayers.length}/11 — ${match.team2 || ''} ${lineups.awayPlayers.length}/11`,
			)
		}
				...
				batch.map(async (match, idx) => {
					...
					try {
						...
						const details = match.url
							? await parseMatchFull(detailPage, match.url, match)
							: {
									firstHalfHome: 0,
									firstHalfAway: 0,
									secondHalfHome: match.score1,
						...
						return {
							...
							score1: match.score1,
							score2: match.score2,
							firstHalfHome: details.firstHalfHome,
							firstHalfAway: details.firstHalfAway,
							secondHalfHome: details.secondHalfHome,
							secondHalfAway: details.secondHalfAway,
							scoreDisplay,
							url: match.url,
							homeGoals: details.homeGoals,
							awayGoals: details.awayGoals,
							ownGoals: details.ownGoals,
